...
player_image = pygame.image.load('вероніка.jpg')
...
def reset_game_state():
    global grid_cells, current_cell, stack, colors, color, player_pos, start_time, stars, collected_stars, first_move_made, game_won, valid_moves, total_moves, move_count
...
def generate_stars():
    ...
    while len(star_positions) < 5:
        ...
        if (x, y) != (0, 0) and (x, y) != (cols - 1, rows - 1) and (x, y) not in star_positions:
...
def draw_statistics_window(elapsed_time, collected_stars, accuracy, move_count):
    ...
    text = font.render('Level Complete!', True, pygame.Color('black'))
    ...
    if not fullscreen_toggle_pressed:
        if not game_won:
            ...
            if first_move_made:
                elapsed_time = time.time() - start_time
                accuracy = (valid_moves / total_moves) * 100 if total_moves > 0 else 100
                draw_sidebar(elapsed_time, collected_stars, accuracy, move_count)
